...
class CleanupService:
    ...
    @traced_cache(
        operation="working_memory.cleanup", collection="quimera_working_memory"
    )
    async def cleanup_expired(
        ...
        deleted = await self._store.delete_expired_points(
...
def _assert_cleanup_collection(collection_name: str) -> None:
    if collection_name == "quimera_working_memory" or collection_name.startswith(
        "quimera_working_memory_test_"
    ):
        return
