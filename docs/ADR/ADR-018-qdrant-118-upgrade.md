# ADR-D2P-018: Qdrant 1.18.x Local-First Upgrade

Status: Accepted-D2P

Supersession note 2026-06-07: active server patch target is now
`qdrant/qdrant:v1.18.2`, with server/client compatibility accepted by `1.18.x`
family per `docs/pkd/PKD-D2P-00Y-qdrant-118-family-parity.md`.
...
Qdrant Server 1.18.2 is the current patch release in the 1.18 family. It
inherits the 1.18.1 local-first fixes and becomes the active server target
because the project policy is to use the latest stable patch within the accepted
minor family. The lockfile currently resolves
...
Target Qdrant Server `qdrant/qdrant:v1.18.2` for local-first runtime
evaluation.
...
This ADR does not reinterpret Qdrant 1.18.0 or 1.18.1 benchmark conclusions as
1.18.2 results. It accepts 1.18.2 as a reversible local target because the patch
fixes reduce operational risk and can be rolled back if readiness, smoke or
benchmark results regress.

## Server/Client Version Policy

- Server target: `1.18.2`.
- Docker image: `qdrant/qdrant:v1.18.2`.
...
2. Target latest Qdrant Server 1.18.x with `qdrant-client>=1.18`.
...
patch-level server fixes to be evaluated without a production migration. The
active patch target is governed by PKD-D2P-00Y.
...
- Q18-07 benchmark conclusions for 1.18.0/1.18.1 must be repeated for 1.18.2.
...
1. Stop Qdrant 1.18.2 container.
2. Change `server_image` in `infra/qdrant/version_contract.yaml`.
3. Change compose image to an explicit rollback tag such as
   `qdrant/qdrant:v1.18.1`, `qdrant/qdrant:v1.18.0` or historical
...
- Save 1.18.2 readiness output in `evaluation/results/qdrant_1182_readiness.json`.
- Repeat Qwen3 benchmark with server version `1.18.2` recorded.
