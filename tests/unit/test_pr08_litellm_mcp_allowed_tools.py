...
def test_agentic0_allowed_tools_are_explicit_and_non_destructive() -> None:
    ...
    assert "*" not in tools
    assert not any(
        "delete" in tool or "recreate" in tool or "store" in tool for tool in tools
    )
    assert "postgres_agent_state_upsert" not in tools
    assert (
        "postgres_agent_state_upsert" in cfg.agentic0_tool_policy.optional_write_tools
    )
    ...
    assert (
        "working_memory_point_upsert" in cfg.agentic0_tool_policy.optional_write_tools
    )
